...
const RUTAS_SUBMODULOS = [
  ...
  {
    modulo: 'catalogo',
    submodulo: 'catalogo-documentos',
    rutas: ['/catalogo/explorar/documentos'],
  },
  { modulo: 'catalogo', submodulo: 'catalogo-tablas', rutas: ['/catalogo/explorar/tablas'] },
  {
    modulo: 'catalogo',
    submodulo: 'catalogo-externos',
    rutas: ['/catalogo/explorar/catalogos-externos'],
  },
  { modulo: 'catalogo', submodulo: 'catalogo-cargar', rutas: ['/catalogo/cargar-archivos'] },
  // En esta instancia el directorio de páginas es `mis-recursos`; se cubren los
  // dos nombres para no romper los enlaces que upstream ya migró a `mis-archivos`.
  {
    modulo: 'catalogo',
    submodulo: 'catalogo-mis-archivos',
    rutas: ['/catalogo/mis-archivos', '/catalogo/mis-recursos'],
  },
  ...
  { modulo: 'consulta', submodulo: 'consulta-tablas', rutas: ['/consulta/tablas'] },
  {
    ...
    rutas: ['/geocontenidos/mapas', '/mapas'],
  },
  {
    modulo: 'geocontenidos',
    submodulo: 'geocontenidos-panoramas',
    rutas: ['/geocontenidos/panoramas', '/panoramas'],
  },
  {
    modulo: 'geocontenidos',
    submodulo: 'geocontenidos-geohistorias',
    rutas: ['/geocontenidos/geohistorias', '/geohistorias'],
  },
  {
    modulo: 'geocontenidos',
    submodulo: 'geocontenidos-tableros',
    rutas: ['/geocontenidos/tableros', '/tableros'],
  },
  {
    modulo: 'geocontenidos',
    submodulo: 'geocontenidos-importar',
    rutas: ['/geocontenidos/importar-datos'],
  },
  {
    modulo: 'levantamiento',
    submodulo: 'levantamiento-proyectos',
    rutas: ['/levantamiento/proyectos'],
  },
  {
    modulo: 'levantamiento',
    submodulo: 'levantamiento-aportes',
    rutas: ['/levantamiento/aportes'],
  },
  {
    modulo: 'levantamiento',
    submodulo: 'levantamiento-descargas',
    rutas: ['/levantamiento/descargas'],
  },
  {
    modulo: 'levantamiento',
    submodulo: 'levantamiento-revision-proyectos',
    rutas: ['/levantamiento/revision-proyectos'],
  },
  {
    modulo: 'levantamiento',
    submodulo: 'levantamiento-revision-aportes',
    rutas: ['/levantamiento/revision-aportes'],
  },
  {
    modulo: 'levantamiento',
    submodulo: 'levantamiento-revision-descargas',
    rutas: ['/levantamiento/revision-descargas'],
  },
...
export default defineNuxtRouteMiddleware(async (to) => {
  const { configuracion, submodulos, cargarConfiguracionModulos } = useConfiguracionModulos();
  ...
  for (const [ruta, habilitada] of Object.entries(reglasBanderas)) {
    if (coincideRuta(to.path, ruta) && !habilitada) {
      return noDisponible();
    }
  }

  const reglasModulos = Object.entries(RUTAS_RAIZ_MODULOS) as Array<[ModuloGestionable, string]>;
  for (const [modulo, rutaRaiz] of reglasModulos) {
    const rutasModulo = [rutaRaiz, ...(RUTAS_ADICIONALES_MODULOS[modulo] || [])];
    if (rutasModulo.some((ruta) => coincideRuta(to.path, ruta)) && !configuracion.value[modulo]) {
      return noDisponible();
    }
  }
  ...
  for (const seccion of RUTAS_RAIZ_SECCIONES) {
    if (to.path === seccion.ruta || to.path === `${seccion.ruta}/`) {
      const submodulo = seccion.submodulos.find((id) => submodulos.value[id]);
  ...
  const rutasCoincidentes = RUTAS_SUBMODULOS.filter((configuracionRuta) =>
    ...
    (a, b) =>
      Math.max(...b.rutas.map((ruta) => ruta.length)) -
      Math.max(...a.rutas.map((ruta) => ruta.length))
  );
